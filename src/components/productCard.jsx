...
export default function ProductCard(props){
    ...
    return(
        <Link to={`/overview/${product.productId}`} className="w-[350px] h-[400px] m-4 p-4 rounded-lg shadow-lg bg-white overflow-hidden hover:[&_.main-image]:opacity-0 relative ">
            
            <div className="bg-white absolute top-0 left-0 w-full">
            <img src ={product.images[1]} alt={product.images} className="h-[250px] object-cover"/>
            </div>
            <div className="bg-white main-image absolute top-0 left-0 transition-opacity duration-500">
            <img src ={product.images[0]} alt={product.images} className="h-[250px] object-cover"/>
            </div>
            <div className="h-[150px] w-full absolute bottom-0 flex border justify-center flex-col p-2">
